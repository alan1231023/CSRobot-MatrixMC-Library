...
CANopenNodeRenesas* canopenNodeSTM32;   // It will be set by canopen_app_init and will be used
...
int canopen_app_init(CANopenNodeRenesas* _canopenNodeSTM32)
{
...
int canopen_app_resetCommunication()
{
    ...
    err =
        CO_LSSinit(CO, &lssAddress, &canopenNodeSTM32->desiredNodeID, &canopenNodeSTM32->baudrate);
    if (err != CO_ERROR_NO) {
...
void canopen_app_process()
{
    ...
    if ((time_current - time_old) > 0) {   // Make sure more than 1ms elapsed
        ...
        if (reset_status == CO_RESET_COMM) {
            ...
            NVIC_SystemReset();   // Reset the Arduino R4 Microcontroller
        }
    }
}
