 ...
 * ###Usage
 *
 * Usage of the CANopen LSS master is demonstrated in file
...
typedef enum {
    ...
    CO_LSSmaster_INVALID_STATE =
        -3, /**< State machine not ready or already processing a request */
    CO_LSSmaster_SCAN_NOACK =
        -4, /**< No node found that matches scan request */
    CO_LSSmaster_SCAN_FAILED =
        -5, /**< An error occurred while scanning. Try again */
    CO_LSSmaster_OK_ILLEGAL_ARGUMENT =
        -101, /**< LSS success, node rejected argument because of non-supported
                 value */
    CO_LSSmaster_OK_MANUFACTURER =
        -102, /**< LSS success, node rejected argument with manufacturer error
                 code */
...
typedef struct {
    ...
    volatile void *CANrxNew; /**< Indication if new LSS message is received from
                                CAN bus. It needs to be cleared when received
    ...
    void (*pFunctSignal)(
        void *object);       /**< From CO_LSSmaster_initCallbackPre() or NULL */
 ...
 * value must be selected "as high as necessary and as low as possible". CiA
 * does neither specify nor recommend a value.
 *
 ...
 *
 * Function must be called cyclically until it returns !=
 * #CO_LSSmaster_WAIT_SLAVE Function is non-blocking.
 *
...
CO_LSSmaster_return_t CO_LSSmaster_configureBitTiming(
    CO_LSSmaster_t *LSSmaster, uint32_t timeDifference_us, uint16_t bit);
...
typedef enum {
    ...
    CO_LSSmaster_FS_MATCH =
        2, /**< Full 32 bit value is given as argument, just verify */
...
typedef struct {
    CO_LSSmaster_scantype_t
        scan[4]; /**< Scan type for each part of the LSS address */
    CO_LSS_address_t
        match; /**< Value to match in case of #CO_LSSmaster_FS_MATCH */
 ...
 * - a (more or less) arbitrary node is selected and ready for node ID
assingment
 ...
 * - verification
 *
 * Most of the time, those are used in combination. Consider the following
example:
 ...
 * @return #CO_LSSmaster_ILLEGAL_ARGUMENT,  #CO_LSSmaster_INVALID_STATE,
 * #CO_LSSmaster_WAIT_SLAVE, #CO_LSSmaster_SCAN_FINISHED,
#CO_LSSmaster_SCAN_NOACK,
 * #CO_LSSmaster_SCAN_FAILED
 */
CO_LSSmaster_return_t
CO_LSSmaster_IdentifyFastscan(CO_LSSmaster_t *LSSmaster,
                              uint32_t timeDifference_us,
                              CO_LSSmaster_fastscan_t *fastscan);
