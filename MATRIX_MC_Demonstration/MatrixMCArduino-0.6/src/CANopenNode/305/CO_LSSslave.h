...
typedef struct {
    ...
    uint16_t
        *pendingBitRate; /**< Bit rate value that is temporarily configured */
    uint8_t *pendingNodeID; /**< Node ID that is temporarily configured */
    uint8_t activeNodeID;   /**< Node ID used at the CAN interface */

    volatile void *sendResponse; /**< Variable indicates, if LSS response has to
                                    be sent by mainline processing function */
    CO_LSS_cs_t service; /**< Service, which will have to be processed by
                            mainline processing function */
    uint8_t CANdata[8];  /**< Received CAN data, which will be processed by
                            mainline processing function */

#if ((CO_CONFIG_LSS) & CO_CONFIG_FLAG_CALLBACK_PRE) || defined CO_DOXYGEN
    void (*pFunctSignalPre)(
        void *object); /**< From CO_LSSslave_initCallbackPre() or NULL */
    void *functSignalObjectPre; /**< Pointer to object */
#endif
    bool_t (*pFunctLSScheckBitRate)(
        void *object,
        uint16_t bitRate); /**< From CO_LSSslave_initCheckBitRateCallback() or
                              NULL */
    void *functLSScheckBitRateObject; /** Pointer to object */
    void (*pFunctLSSactivateBitRate)(
        void *object,
        uint16_t delay); /**< From CO_LSSslave_initActivateBitRateCallback() or
                            NULL. Delay is in ms */
    void *functLSSactivateBitRateObject; /** Pointer to object */
    bool_t (*pFunctLSScfgStore)(
        void *object,
        uint8_t id,
        uint16_t
            bitRate); /**< From CO_LSSslave_initCfgStoreCallback() or NULL */
 ...
 * timing parameters" is used. The callback function needs to check if the new
 * bit rate is supported by the CANopen device. Callback returns "true" if
 * supported. When no callback is set the LSS slave will no-ack the request,
 * indicating to the master that bit rate change is not supported.
 *
 * @param LSSslave This object.
 * @param object Pointer to object, which will be passed to
 * pFunctLSScheckBitRate(). Can be NULL
 * @param pFunctLSScheckBitRate Pointer to the callback function. Not called if
 * NULL.
 */
 ...
 * arrived.
 * According to DSP 305 6.4.4, the delay has to be applied once before and once
 * after switching bit rates. During this time, a device mustn't send any
 * messages.
 *
 * @param LSSslave This object.
 * @param object Pointer to object, which will be passed to
 * pFunctLSSactivateBitRate(). Can be NULL
 * @param pFunctLSSactivateBitRate Pointer to the callback function. Not called
 * if NULL.
 */
 ...
 *
 * Function initializes callback function, which is called when "store
 * configuration" is used. The callback function gives the user an event to
 * store the corresponding node id and bit rate to NVM. Those values have to be
 * supplied to the init function as "persistent values" after reset. If callback
 * returns "true", success is send to the LSS master. When no callback is set
 * the LSS slave will no-ack the request, indicating to the master that storing
 * is not supported.
 *
 * @param LSSslave This object.
 * @param object Pointer to object, which will be passed to pFunctLSScfgStore().
 * Can be NULL
 * @param pFunctLSScfgStore Pointer to the callback function. Not called if
 * NULL.
 */
