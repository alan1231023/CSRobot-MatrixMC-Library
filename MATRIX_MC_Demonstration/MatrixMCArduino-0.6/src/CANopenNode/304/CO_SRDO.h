 ...
 * The second message must be bitwise inverted. The delay between the two
 * messages and between each message pair is monitored. The distinction between
 * sending and receiving SRDO is made at runtime (for PDO it is compile time).
 * If the security protocol is used, at least one SRDO is mandatory.
 */


/**
 * SRDO communication parameter. The same as record from Object dictionary
 * (index 0x1301-0x1340).
 */
typedef struct {
    ...
    /** Communication object identifier for message received. Meaning of the
       specific bits:
...
typedef struct {
    /** Actual number of mapped objects from 0 to 16. Only even numbers are
    allowed. To change mapped object, this value must be 0. */
    uint8_t numberOfMappedObjects;
    /** Location and size of the mapped object.
        Even index is the normal object. Odd index is the inverted object. Bit
       meanings `0xIIIISSLL`:
...
typedef struct {
    CO_NMT_internalState_t
        *operatingState; /**< pointer to current operation state */
    CO_NMT_internalState_t operatingStatePrev; /**< last operation state */
    uint8_t *configurationValid; /**< pointer to the configuration valid flag in
                                    OD */
 ...
 * @param operatingState Pointer to variable indicating CANopen device NMT
 * internal state.
 ...
 * @param SDO SDO object.
 * @param nodeId CANopen Node ID of this device. If default COB_ID is used,
 * value will be added.
 * @param defaultCOB_ID Default COB ID for this SRDO (without NodeId).
 * @param SRDOCommPar Pointer to _SRDO communication parameter_ record from
 * Object dictionary (index 0x1301+).
 ...
 * Initialize SRDO callback function.
 *
 * Function initializes optional callback function, that is called when SRDO
 * enters a safe state. This happens when a timeout is reached or the data is
 * inconsistent. The safe state itself is not further defined. One measure, for
 * example, would be to go back to the pre-operational state Callback is called
 ...
 * in CO_SRDO_process(). After the transmission the timer is reset to the full
 * refresh time.
 *
 * @param SRDO This object.
 ...
 *  This function also configures the SRDO on operation state change to
 * operational
 *
 * @param SRDO This object.
