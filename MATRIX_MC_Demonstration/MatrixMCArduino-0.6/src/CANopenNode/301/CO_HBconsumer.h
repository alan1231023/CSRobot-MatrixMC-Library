...
#define CO_CONFIG_HB_CONS                                                      \
    (CO_CONFIG_HB_CONS_ENABLE | CO_CONFIG_GLOBAL_FLAG_CALLBACK_PRE |           \
...
typedef enum {
    ...
    CO_HBconsumer_UNKNOWN =
        0x01U, /**< Consumer enabled, but no heartbeat received yet */
 ...
 *
 * Function initializes optional callback function, which is called when the
 * node state changes from active to timeout. Function may wake up external
 * task, which handles this event.
 *
 * @param HBcons This object.
 * @param idx index of the node in HBcons object
 * @param object Pointer to object, which will be passed to pFunctSignal(). Can
 ...
 *
 * Function initializes optional callback function, which is called when a
 * bootup message is received from the remote node. Function may wake up
 * external task, which handles this event.
 *
 * @param HBcons This object.
 * @param idx index of the node in HBcons object
 * @param object Pointer to object, which will be passed to pFunctSignal(). Can
